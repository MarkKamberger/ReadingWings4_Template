...
function Timeline(animationsArray, numFrames, loop, startFunc, stopFunc)
{
  ...
  this.frameCounter = 0;
  this.stepper(0);
  this.stopFunction = stopFunc;
  this.startFunction = startFunc;
}

Timeline.prototype.play = function(startFrame)    // optional startFrame
{
  setTimeout(this.startFunction, 10);
...
Timeline.prototype.stop = function()
{
  ...
  if (this.timer) {
	...
	if (i != oldTimer) {
		result = true;
		oldTimer = i;
		setTimeout(this.stopFunction, 50);
	}
  }
  this.mode = modes.STOPPED;
  return result;
